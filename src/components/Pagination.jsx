...
const Pagination = ({ queries }) => {
  ...
  const { page, setPage, navigate } = useContext(Context);
  ...
  const paginationClickHandler = (startIndex) => {
    // console.log(queries.nextPage[0]?.count);
    setPage(startIndex);
  ...
  return (
    <div className="flex flex-col items-center py-14 max-w-[700px]">
      <div className="relative text-[#4285f4]">
        ...
        {queries?.request[0]?.startIndex !== 91 && (
          <div
        ...
        {pagination.map((p) => (
          <span
            ...
            className={`cursor-pointer ${
              page === p.startIndex ? "text-black dark:text-white" : ""
            }`}
          >
            {p.page}
          </span>
